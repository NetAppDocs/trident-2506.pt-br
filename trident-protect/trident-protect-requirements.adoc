...
---
= Requisitos de proteção Trident
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
== O Trident Protect garante a compatibilidade do cluster Kubernetes.
...
* Certifique-se de que o cluster no qual você está instalando o Trident Protect esteja configurado com um controlador de snapshots em execução e os CRDs relacionados.  Para instalar um controlador de snapshots, consulte https://docs.netapp.com/us-en/trident/trident-use/vol-snapshots.html#deploy-a-volume-snapshot-controller["estas instruções"] .
...
Você pode configurar o Trident Protect 24.10 para gerenciar o congelamento e descongelamento do sistema de arquivos da máquina virtual durante as operações de proteção de dados.link:https://docs.openshift.com/container-platform/4.16/virt/install/installing-virt.html["configurando a virtualização"^] e então usando o seguinte comando:
...
O Trident Protect exige que você configure pelo menos um backend de armazenamento que suporte replicação tanto para o cluster de origem quanto para o de destino.  Se os clusters de origem e destino forem os mesmos, o aplicativo de destino deverá usar um backend de armazenamento diferente do aplicativo de origem para obter a melhor resiliência.
